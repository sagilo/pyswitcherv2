...
def unpack(num_bytes):
    ...
    return struct.unpack(fmt, num_bytes)[0]
...
class Switcher:
    ...
    @staticmethod
    def __update_request_constants(header):
        header[0:1] = pack(-2, 1)
    ...
    def __generate_control_request(self, is_on, time_minutes, session):
        CONTROL_COMMAND = 513
    ...
    def __send_control(self, is_on, time_minutes, session):
        request = self.__generate_control_request(is_on, time_minutes, session)
...
def extract_credentials_from_pcap(pcap_file, is_debug):
    ...
    try:
        ...
        raise Exception("Error parsing pcap file (invalid pcap file?): %s" % exception)
...
def write_credentials(credentials, credentials_file):
    data = {
        ...
        "device_pass": credentials.device_pass(),
        "ip": credentials.switcher_ip()
    }
